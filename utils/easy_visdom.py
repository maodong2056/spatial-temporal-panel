...
class EasyVisdom:
    ...
    def vis_scalar(self, step, train_statistics, val_statistics):
        ...
        if step == self.start_i:
            ...
            for j, win in enumerate(self.wins):
                self.vis.line(X=np.array([step]), Y=epoch_results[j][0, None], win=win, update='append')
                self.vis.line(X=np.array([step]), Y=epoch_results[j][1, None], win=win, update='append')
    ...
    def vis_images(self, step,
                   train_images=None, val_images=None, im_titles=('input', 'label', 'prediction'), show_interval=5):
        """
        Arguments:
            + step: current step
